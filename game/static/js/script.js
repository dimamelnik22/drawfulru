...
function pencil (){
    ...
    canvas.onmousedown = function (e){
        ...
        curY = e.clientY - 2*canvas.offsetTop;
        hold = true;
...
function circle (){
    ...
    canvas.onmousemove = function (e){
        if (hold){
            ...
            if (fill_value)
                ctx.fill();
            canvas_data.circle.push({ "startx": prevX, "starty": prevY, "radius": curX - prevX, "thick": ctx.lineWidth,
...
function save (gameid){
    ...
    pid = "{{game.id}}";
    $.post("/room/"+gameid+"/draw/",{  save_cdata: data, save_image: image });
